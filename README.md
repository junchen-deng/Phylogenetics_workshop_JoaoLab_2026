...
If IQTREE runs successfully, it will produce a long series of logs on the screen. The output files are generated in the location where you run IQTREE, if no output path was specified. In our case, this will be the IQTREE installation folder. You can also find the output of the run I did in the folder **IQTREE_result** in this repository. 

Launch the software **FigTree**. Go to **File**, **Open...**. Choose to open the file **Ophio_nucl.treefile**, which stores the tree in **Newick** format. In the pop-up window, set the name to **bootstrap**. Select the branch leading to the two outgroups, *T. inflatum* and *T. ophioglossoide*, and click **Reroot** on the top panel. Go to **Trees** on the left panel, tick the box of **Order nodes** and select **decreasing**. Go to **Tip labels** and increase the **Font size**. Tick the box of **Branch Labels**. Choose **bootstrap** for **Display** and increase the font size a bit. Now the tree should look prettier. Go to **File**, **Export PDF...** to save your tree for future editing. (This tree is also inside the folder **IQTREE_result** in this repository under the name **Ophio_nucl.pdf**. Is your tree similar to what I did?)

Take a look at the tree. Where is JPMA317? Is this what you expected? Look at the bootstrap numbers. Are you confident with this topology? Compare the topology of other taxa with the *Ophiocordyceps* tree in the figure '**Zhuang et al. 2005 Figure 3.png**. Are they similar?
